...
class SimpleTree:
    ...
    def EvenTrees(self):
        disconnect_list = []
        if (self.Count() % 2 != 0) or (self.Root is None):
            return disconnect_list
        def tree_count(node, disconnect_list):
            count = 1
            for child in node.Children:
                branch_len = tree_count(child, disconnect_list)
                count += branch_len
            if node.Children == []:
                return 1
            if count % 2 == 0:
                if node.Parent:
                    disconnect_list.append(node.Parent)
                    disconnect_list.append(node)
                return 0
            return count
